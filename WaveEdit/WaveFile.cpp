...
// Play wave file.
void
WaveFile::play(int beginning, int ending)
{
    if (hdr==NULL) {
        return;
    }
    ...
    WaveFile * w2 = new WaveFile(numChannels, sampleRate, 
		...
		PlaySound((LPCWSTR)hdr, NULL, SND_MEMORY | SND_ASYNC | SND_NODEFAULT);
	
	/*else {

		for (t = 0; t < beginning; t++) {
...
WaveFile *
WaveFile::tone(int frequency, int msecs, int beginning, int ending) {
    ...
    for (; t<ending; t++) {
			...
			freq = get_sample(t);
			short value = 32767*sin(3.1415*freq*t/sampleRate);
			
			w2->add_sample(value);
		/*}else {
			freq = (w2->get_sample(t) / 2);
			short value = 32767*tan(3.1415*freq*t/sampleRate);
			w2->add_sample(value);
		}*/
...
WaveFile * 
WaveFile::multiply_freq(double k, int durationms, int beginning, int ending)
{
	WaveFile * w2 = new WaveFile(numChannels, sampleRate,
		bitsPerSample);
...
WaveFile *
WaveFile::reversal(int beginning, int ending)
{
...
WaveFile * 
WaveFile::echo(float echoAmount, float delayms, int beginning, int ending)
{
...
WaveFile::insert_fragment(WaveFile *fragment, int index){
	if (fragment->hdr==NULL) {
        return 0;
    }
	WaveFile * wave = new WaveFile(fragment->numChannels, fragment->sampleRate, fragment->bitsPerSample);
	//index = index * fragment->sampleRate / 1000;
	int count, fragCounter;
	for(count = 0; count < index; count++){
		wave->add_sample(get_sample(count));
	}
	for(fragCounter = 0; fragCounter < fragment->lastSample; fragCounter++){
		wave->add_sample(fragment->get_sample(fragCounter));
	}
	for(;count <lastSample; count++){
		wave->add_sample(get_sample(count));
	}
	wave->updateHeader();
	return wave;
